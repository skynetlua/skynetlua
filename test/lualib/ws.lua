...
function User:send(name, proto)
    skynet.fork(function(name, proto)
        assert(type(name) == "string", "User:send name must be string")
        assert(#name<256, "User:send name string too more")
        local msg_idx = self.msg_idx
        if msg_idx > 255 then
            msg_idx = 1
        end
        local data = string.char(msg_idx)..string.char(#name)..name
        self.msg_idx = msg_idx+1
        if proto then
...
function ws.dispatch(req, res)
    local code, resp = Websocket.handshake(req.header)
    if code then
